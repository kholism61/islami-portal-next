...
if (isHomePage && categoryGrid && typeof window.__PORTAL_ARTICLE_STORE__ !== "undefined") {
  ...
  const renderCategoryCard = (name, data) => {
    ...
    const thumbPath = rawThumb || getCategoryThumb(name) || "assets/images/default.jpg";
    const thumbUrl = new URL(thumbPath, document.baseURI);
    if (name === "politik" && !thumbUrl.searchParams.has("v")) {
      thumbUrl.searchParams.set("v", "20260316");
    }
    img.src = thumbUrl.toString();
    img.alt = getLocalizedCategory(name, getSiteLang());
    img.onerror = () => {
      img.onerror = null;
      img.src = new URL("assets/images/default.jpg", document.baseURI).toString();
    };
